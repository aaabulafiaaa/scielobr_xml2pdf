...
This paper is organized as follows. Section ef{sec:work} describes several previous works existing in literature, related to seismic attributes and high performance computing. Section ef{sec:method} explains the curvature attribute method implemented in this work. Sections ef{sec:cpu} and Section ef{sec:gpu} describe technical details about CPU and GPU implementations, respectively. Results of our experiments are presented in Section ef{sec:results}. Finally, Section ef{sec:conclusions} presents some conclusion remarks.
...
Generally, the method is based on three principal steps (Figure ef{fig:method}). Firstly, seismic reflectors are properly described as level surfaces, through the computation of a horizon identifier attribute. Secondly, normal fields along X, Y and Z directions are estimated based on the gradient of this attribute. Finally, normal field partial derivatives are calculated in order to find desired volumetric curvature attributes, using a differential geometry approach.
...
Let (x,y,z) = 0$ be an implicit surface, $
abla{F}$ the gradient vector and (F)$ the hessian matrix at a given point. In this case, we have:

\begin{equation} \label{eq:notation}
\begin{array}{cc}

abla{F} =  \left(rac{\partial F}{\partial x} \mbox{  } rac{\partial F}{\partial y} \mbox{  } rac{\partial F}{\partial z} ight) = (F_x \mbox{ } F_y \mbox{ } F_z), \\
H(F) = \left(\begin{array}{rrr} rac{\partial^2 F}{\partial x^2} \mbox{  } &  rac{\partial^2 F}{\partial x \partial y} \mbox{  } & rac{\partial^2 F}{\partial x \partial z}\
				rac{\partial^2 F}{\partial y \partial x} \mbox{  } &  rac{\partial^2 F}{\partial y^2} \mbox{  } & rac{\partial^2 F}{\partial y \partial z} \
				rac{\partial^2 F}{\partial z \partial x} \mbox{  } &  rac{\partial^2 F}{\partial z \partial y} \mbox{  } & rac{\partial^2 F}{\partial z^2} nd{array} ight) = \\ \left(\begin{array}{rrr} F_{xx} \mbox{ } & F_{xy} \mbox{ } & F_{xz} \
					   F_{yx} \mbox{ } & F_{yy} \mbox{ } & F_{yz} \
					   F_{zx} \mbox{ } & F_{zy} \mbox{ } & F_{zz} nd{array} ight) = 
abla{(
abla{F})}
nd{array}
nd{equation}
...
To properly represent normal orientation along implicit surfaces and curvature signal convention, it is necessary to change the gradient according to its $ component signal:
...
Thus, we can define an adapted hessian matrix ^*(F)$ \cite{martinsthesis}, by the following expression:

\begin{equation} \label{eq:novo}
\begin{array}{cc}
N(F) = (N_x \mbox{ } N_y \mbox{ } N_z),\\
H^*(F) = \left(\begin{array}{rrr} rac{\partial N_x}{\partial x} \mbox{  } &  rac{\partial N_x}{\partial y} \mbox{  } & rac{\partial N_x}{\partial z}\
				rac{\partial N_y}{\partial x} \mbox{  } &  rac{\partial N_y}{\partial y} \mbox{  } & rac{\partial N_y}{\partial z}\
				rac{\partial N_z}{\partial x} \mbox{  } &  rac{\partial N_z}{\partial y} \mbox{  } & rac{\partial N_z}{\partial z}\ nd{array} ight)
nd{array}
nd{equation}


At this point, gaussian curvature $ can be obtained using the following expression

\begin{equation}\label{eq:kG}
k_G =  rac{ \left|\begin{array}{rr} H^*(F) & N(F)^T \ N(F) & 0 nd{array} ight|} {|N(F)|^4}
\
nd{equation}
...
k_M =  rac{ N(F) * H^*(F) * N(F)^T - |N(F)|^2Trace(H^*)} {2|N(F)|^3}
\
nd{equation}

Once these two measures are defined for a given point, we can obtain maximum and minimum curvatures,  $ and $, by the classic equation
...
Curvature attributes estimate is the final and most compute-intensive step. Indeed, it involves, for each voxel, the computation of a 4x4 determinant (Equation ef{eq:kG}), composed by matrix ^*(F)$, normal vector (F)$ and the last element, zero. Also, the 9 partial derivatives of  ^*(F)$ are obtained through convolution of normal field volumes with gaussian derivative filters.
...
Although most implementations of stencil computation access neighbors  in a cross pattern, our implementation uses a 3D grid shape to obtain better continuity of seismic features on the final result (Figure ef{fig:kernel}).
...
On the GPU, the maximum performance is only achieved if you keep everything in registers, registers are the fasted storage on GPUs with a peak of bandwidth of 10TB/s (1331 flops $ightarrow$ 16 bytes per multiply and add $ightarrow$ 3 loads 1 store)  on Tesla M2090 \cite{nvidia}.
...
We maximize data reuse using a 3D circular buffer around the stencil points using shared memory for the curvature estimate step. To process a single voxel we need, for example, a 5x5x5 region. Thus we load, before the loop, four planes and for each new plane the  + 2$ overwrites the  - 2$ old plane. Figure ef{fig:circular_buffer} shows how circular buffers works. The implementation overview can be seen on Figure ef{fig:overview}.


\begin{figure}
  \centering
  \includegraphics[width=\columnwidth]{circular_buffer_layers_smaller.pdf}
  \caption{Circular buffer on shared memory. $ is the slowest dimension. The next iteration will overwrite the red plan }
...
The resultant curvature for a 32 bits volume with 244MB (291 inline, 476 crossline and 462 time slices) is presented in Figure ef{fig:horizonResults}. The computational cost is evaluated in Table ef{table:volumeresults}. For the 5x5x5 operator size, which is a value commonly used by interpreters, the curvature volume is obtained in nearly 0.5 second by GPU implementation with a speed-up of 11.2 times in comparison to CPU implementation with OpenMP (12 threads). For the largest derivative operator size (17x17x17) the GPU achieves a speed-up of 114.2 times in relation to CPU sequential execution time and a speed-up of 8.63 times in comparison to CPU with OpenMP (12 threads).
...
\begin{tabular}{@{}cccc@{}}
	oprule
Operator size & \begin{tabular}[c]{@{}c@{}}CPU sequential\  time (s)nd{tabular} & \begin{tabular}[c]{@{}c@{}}CPU with OpenMP\ time (s)nd{tabular} & \begin{tabular}[c]{@{}c@{}}GPU\ time (s)nd{tabular} \ \midrule
...
\begin{figure*}[ht!]
  ...
  \caption{Volumetric curvature attribute $ mapped along an tracked horizon, using 5x5x5 (a), 7x7x7 (b), 11x11x11 (c) and 17x17x17 (d) derivative operator sizes. As the operator size increases, subtle details trend to disappear, and only main structural features remains on final result.}
...
\begin{tabular}{@{}cccc@{}}
	oprule
Operator size & \begin{tabular}[c]{@{}c@{}}GPU without \ shared buffer \  time (s)nd{tabular} & \begin{tabular}[c]{@{}c@{}}GPU with \ 2D shared buffer\ time (s)nd{tabular} & \begin{tabular}[c]{@{}c@{}}GPU with \ circular 3D \ shared buffer\ time (s)nd{tabular} \ \midrule
...
\begin{tabular}{@{}ccccc@{}}
	oprule
Operator size & \begin{tabular}[c]{@{}c@{}}Number \of slice\  requiredsnd{tabular} & \begin{tabular}[c]{@{}c@{}}CPU \ with OpenMP\ time (ms)nd{tabular} & \begin{tabular}[c]{@{}c@{}}GPU \  optimized\ time (ms)nd{tabular} & \begin{tabular}[c]{@{}c@{}} Memory transfer\ host  $\leftrightarrow$ device \ time (ms)nd{tabular}\ \midrule
